...
module.exports.run = async (Client, message, args, prefix) => {
    ...
    if(!Description) return message.reply('Give Me Some Text')

    // create an embed
    const embed = new discord.MessageEmbed()
    .setColor("RANDOM") // the color of the embed 
    .setDescription(`${Description} ${reactRole} `) // desc of the embed
    ...
    const reactionFilter = (reaction, user) => (args[1]).includes(reaction.emoji.name)
